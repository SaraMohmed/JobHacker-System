...
public class Product {
    ...
    public static void main(String[] args) {
        ...
        Product p = new Product();
                p.addDummyData().stream()
                        .filter(product ->p.category == "Books" )


        p.addDummyData().stream()
                .filter(product ->p.category == "Books" && p.price>100 )


        p.addDummyData().stream()
                .filter(product ->p.category == "Toys" && p.price>100 )


//        p.addDummyData().stream()
//                .filter(product -> p.category == "Book")
//                .min(double::);
//
